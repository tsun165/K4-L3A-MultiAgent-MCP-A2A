...
| `order-agent` | `case`, `EvidenceStore`, totals của `payment-agent` (handoff `payment_totals`) | Xác nhận `order_id` qua MCP, lấy `order_ids`/`item_ids`/`seller_ids` từ evidence; rule `canceled_order_paid` (`order_status=canceled` → `ORDER_CANCELED_AFTER_PAYMENT`, platform) và `unavailable_order_paid` (`unavailable` → `ITEM_UNAVAILABLE_AFTER_PAYMENT`, seller); refund = `paid_total_brl − refunded_total_brl`; claim trái với status → `unsupported_claim` | `SpecialistResult` $\to$ `coordinator` | `get_order`, `get_order_items`, `get_sellers` (chỉ khi seller chịu trách nhiệm) |
| `shipment-agent` | `case`, `EvidenceStore` | Chỉ chạy khi claim thuộc nhóm giao hàng và đơn `delivered`. Chọn 1 `shipping_limit_at`/item nằm trong [ngày mua, `opened_at`] (loại dòng nhiễu). `delivered_carrier_at > limit` → `late_delivery_seller` (`SELLER_LATE_HANDOVER`, seller); seller đúng hạn nhưng `delivered_customer_at > estimated_delivery_at` → `late_delivery_logistics` (`CARRIER_DELIVERY_DELAY`, logistics_provider); refund = `freight_value` của item; đúng hạn → `unsupported_claim`. MCP không có shipment id → `shipment_ids=[]` | `SpecialistResult` $\to$ `coordinator` | `get_shipment_summary`, `get_order_items` (freight), `get_order` (ngày mua, chỉ khi limit mâu thuẫn) |
